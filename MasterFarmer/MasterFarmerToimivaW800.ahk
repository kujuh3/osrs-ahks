...
FailSafeDropping:
PixelSearch, Alarivix, Alariviy, 600, 416, 760, 441, 0X0000FF, 1, Fast
	if (ErrorLevel =0) {
	GoTo, DropSeeds
	}
...
PixelSearch, Masterx, Mastery, 18, 33, 780, 523, 0X0087FF, 1, Fast
    if (ErrorLevel =0)    {
...
Random, Thief1x, 5, 12
Random, Thief1y, 5, 12 
Random, RandTimeThieving, 300, 450

PixelSearch, Thiefx, Thiefy, 20, 33, 780, 523, 0X0087FF, 1, Fast
    if (ErrorLevel =0)    {
    MouseGetPos, xpos, ypos
    RandomBezierThieving(xpos, ypos, Thiefx+Thief1x, Thiefy+Thief1y, "T" RandTimeThieving "OT2 OB2 OL2 OR2")
	}
	GoTo, StunCheck

StunCheck:
PixelSearch, StatusTopx, StatusTopy, 45, 59, 140, 75, 0X00C8FF, 1, Fast
	if (ErrorLevel =0) {
	GoTo, DropSeeds
	}
		else {
...
PixelSearch, Seeditx, Seedity, 590, 198, 770, 443, 0X0000FF, 1, Fast
	if (ErrorLevel =0) {
	MouseGetPos, xpos, ypos
...
PixelSearch, Foodx, Foody, 596, 198, 770, 443, 0X4DFF00, 1, Fast
	if (ErrorLevel =0) {
	Random, x2, -6, 6
	Random, y2, -6, 6
...
PixelSearch, Bankkix, Bankkiy, 600, 27, 780, 198, 0XFF0101, 1, Fast
	if (ErrorLevel =0) {
	Random, x2, -4, 4
	Random, y2, -2, 4
	MouseGetPos, xpos, ypos
...
PixelSearch, BankBoothx, BankBoothy, 20, 259, 550, 503, 0XFFFF00, 1, Fast
	if (ErrorLevel =0) {
	Random, x3, 0, 8
	Random, y3, 0, 8
	Random, RandTimeBankBooth, 100, 300
...
PixelSearch, Px, Py, 385, 326, 418, 357, 0X1D1F83, 10, Fast
	if (ErrorLevel =0) {
	MouseGetPos, xpos, ypos
	RandomBezier(xpos, ypos, 460+x4, 340+y4, "T" RandTimeDeposit "OT10 OB10 OL10 OR10")
	Click
	MouseGetPos, xpos, ypos
	RandomBezier(xpos, ypos, 443+x6, 303+y6, "T" RandTimeDeposit2 "OT10 OB10 OL10 OR10")
	Click
	Sleep, 800
	GoTo, BackToNpc
}
...
PixelSearch, Lobox, Loboy, 590, 198, 770, 443, 0X4DFF00, 1, Fast
	if (ErrorLevel =0) {
	MouseGetPos, xpos, ypos
	RandomBezier(xpos, ypos, 668+x5, 89+y5, "T" RandTimeNPC "OT100 OB100 OL100 OR100")
	Click
	GoTo, FailSafeDropping
	}
	else {
